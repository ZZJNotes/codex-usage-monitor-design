...
function TokenUsageSection({ state, locale, formatLocale, onQuery }: {
  ...
  const data = state.status === "ready" ? state.data : state.status === "error" ? state.lastData : null;
  ...
  return <section className="token-section" aria-labelledby="token-heading">
    ...
    {state.status === "error" && <div className="error-banner" role="alert"><div><strong>{t("tokenError")}</strong><span>{state.message}</span></div></div>}
...
export function App() {
  ...
  const [tokenUsage, setTokenUsage] = useState<TokenUsageState>({ status: "loading" });
  const [tokenFilters, setTokenFilters] = useState<TokenUsageFilters>({});
  ...
  const readTokenUsage = useCallback(async (filters: TokenUsageFilters) => {
    try {
      setTokenUsage(await monitorApi.getTokenUsage(filters));
    } catch (error) {
      setTokenUsage({ status: "error", message: errorMessage(error), lastData: null });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    void refresh();
    void readQuota();
    void readTokenUsage(tokenFilters);
    ...
    const tokenTimer = window.setInterval(() => void readTokenUsage(tokenFilters), 3_000);
    return () => {
  ...
  }, [readQuota, readTokenUsage, refresh, refreshHistory, refreshPreferences, tokenFilters]);
