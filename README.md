...
- 通过本机 Codex app-server 只读显示当前 ChatGPT 登录账户的动态命名额度窗口、剩余百分比和重置时间；最近一次有效快照保存在 SQLite，应用重启后仍可恢复；不会读取或复制 OAuth token，也不会切换 Codex 登录。

当前额度能力仅覆盖“当前 Codex 登录账户”的只读视图。独立授权和管理多个 ChatGPT 账户仍依赖 Issue #2 的双真实账户兼容性验证，不能视为 S3 完成。本机 Codex Token 统计、归属、通知、导出和完整发布验收也仍属于后续产品切片。
